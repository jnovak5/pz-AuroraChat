...
function AC.Parsing.ParseMessage(message)
    ...
    if (unStart and unStart < 50) or (posStart and posStart < 50) or (langStart and langStart < 50) then
        ...
        if remainder:match("^[%s~<a-z>]*%[radio%]") then
            _, cutPos = message:find("%[radio%]", cutPos)
            onRadio = true
            remainder = message:sub(cutPos + 1)
        end
        
        if remainder:match("^[%s~<a-z>]*%[emote%]") then
            _, cutPos = message:find("%[emote%]", cutPos)
            emote = true
            remainder = message:sub(cutPos + 1)
        end
        
        if remainder:match("^[%s~<a-z>]*%[Recorder%]") then
            _, cutPos = message:find("%[Recorder%]", cutPos)
            fromRecorder = true
            remainder = message:sub(cutPos + 1)
        end
        
        if remainder:match("^[%s~<a-z>]*%[npc%]") then
            _, cutPos = message:find("%[npc%]", cutPos)
            isNpc = true
            remainder = message:sub(cutPos + 1)
        end
        
        message = message:sub(cutPos + 1)
    else
        -- Fallback to the original exact parsing in case metadata was perfectly intact but UN/POS were missing 
        -- (e.g. just [radio] or [emote] at the start, though rare in AC)
        while true do
            if message:sub(1, 7) == "[radio]" then
                onRadio = true
...
function AC.Parsing.FormatMessage(parsedMessage)
    ...
    -- Append punctuation if missing (REMOVED per user request)
    ...
    -- Fix img tags for vanilla chat log
    message = message:gsub("%[img=([^%]]+)%]", " <IMAGE:%1> ")
