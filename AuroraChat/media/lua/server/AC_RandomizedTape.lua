...
local function AC_CheckItems()
    for i = #toInitializeItems, 1, -1 do
        if toInitializeItems[i][2] <= 0 then
            local item = toInitializeItems[i][1]
            local md = item:getModData()
            if not md["AC_Processed"] and ZombRand(50) == 0 then
                AC_RandomizeTape(item)
            end
            md["AC_Processed"] = true
    ...
    if #toInitializeItems == 0 then
        isChecking = false
        Events.OnTick.Remove(AC_CheckItems)
    end
end

function AC_OnCreateTape(item)
    table.insert(toInitializeItems, {item, 60})
    if not isChecking then
        isChecking = true
        Events.OnTick.Add(AC_CheckItems)
    end
end

function AC_RandomizeTape(item)
    local md = item:getModData()
    local randomStory = stories[ZombRand(#stories) + 1]
    item:setCustomName(true)
    item:setName(randomStory.title)
    for i = 1, #randomStory.messages do
        md["AC_Message" .. i] = randomStory.messages[i]
    end
    md["AC_TapeUsed"] = #randomStory.messages
end
